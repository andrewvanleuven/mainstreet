...
for (i in town_fips) {
  ...
  town_dots <- df %>% 
    ...
    mutate(distance_to = city_fips,
           ...
           inside = if_else(st_intersects(.,sf_downtown, sparse = F),1,0),
           dist_ft = (raw_crow_dist) %>% 
             set_units(., ft) %>%
             as.vector() %>% 
             round(digits = 2)) %>% 
    select(property_id,dist_ft,inside,distance_to) %>% 
    arrange(dist_ft) %>% 
    st_drop_geometry()
...
filtered <- distances %>% 
  ...
  filter(dist_ft == min(dist_ft)) %>% 
  inner_join((df %>% st_drop_geometry()), by = "property_id") %>% 
  write_fst("hidden/datatree/cleaned/datatree_oh04.fst")

write_csv(filtered,"hidden/datatree/cleaned/datatree_clean.csv")
...
ggplot() +
  ...
  geom_sf(data = downtown_edge, color = "blue", size = 2) +
  theme_void()
